...
async def ban(ctx, args):
    ...
    if x == 0:
        ...
        await user.add_roles(y)
        [await user.remove_roles(thisshouldntbeplural) for thisshouldntbeplural in [(await converter2.convert(ctx, str(banroleids[z])))[0] for z in range(0,banroleids.index(x))]]
        await ctx.send("Ban role " + str(y) + " successfully added to user " + args)
    else:
